...
#else
static ssize_t hfsplus_direct_IO(struct kiocb *iocb, struct iov_iter *iter, loff_t offset)
#endif
{
	 ...
	 */
	if (unlikely((iter->type & WRITE) && ret < 0)) {
		loff_t isize = i_size_read(inode);
		loff_t end = offset + iter->count;//iov_length(iov, nr_segs);
