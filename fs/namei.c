...
#ifdef CONFIG_AURALIC_FILELIST
    error = lookup_open(nd, &path, file, op, got_write, opened, nd->name);
#else
...
retry:
    ...
    if(true == vfs_can_access)
    {
        if(0 == error && true == aura_check_path_mache(dfd, pathname, 0))
        {
	        ...
	        name = filename_parentat(dfd, getname(pathname), 0,
				&path, &last, &type);
            if(!IS_ERR(name))
            {
...
retry_deleg:
    ...
    if(0 == error && true == vfs_can_access)
    {
        ...
        {       
            ...
            path_get(&old_path);
            info_old->path = d_path(&old_path, (char *)info_old->buff, PATH_MAX-FILELIST_D_PATH_RESERVE);
            path_put(&old_path);
        ...
        if(true == match_new)
        {
            ...
            path_get(&new_path);
            info_new->path = d_path(&new_path, (char *)info_new->buff, PATH_MAX-FILELIST_D_PATH_RESERVE);
            path_put(&new_path);
