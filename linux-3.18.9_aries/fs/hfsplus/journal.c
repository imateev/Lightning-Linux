...
static int hfsplus_journal_replay(struct super_block *sb)
{
	...
	if (ret == HFSPLUS_JOURNAL_SUCCESS && HFSPLUS_SB(sb)->s_vhdr) {
		...
		if (be16_to_cpu(vhdr->signature) == HFSPLUS_VOLHEAD_SIG) {
		    printk("replay journal for HFSPLUS_VOLHEAD_SIG done\n");
		}
		else if (be16_to_cpu(vhdr->signature) == HFSPLUS_VOLHEAD_SIGX) {
		    set_bit(HFSPLUS_SB_HFSX, &HFSPLUS_SB(sb)->flags);
		    printk("replay journal for HFSPLUS_VOLHEAD_SIGX done\n");
		}
		else {
...
int hfsplus_journaled_check(struct super_block *sb)
{
	...
	*/
	...
	if(jh->start == jh->end) {
		...
		#else
            printk("HFS+-fs: Journal is non empty means inconsistent, mount %s read-only!\n", sb->s_id);
		    return HFSPLUS_JOURNAL_SKIP_REPLAY;
		#endif
	}
...
void hfsplus_journaled_init(struct super_block *sb, struct hfsplus_vh *vhdr)
{
	...
	if (vhdr->attributes & cpu_to_be32(HFSPLUS_VOL_JOURNALED)) {
		...
		jnl->jib_bh = sb_bread(sb, HFSPLUS_SB(sb)->blockoffset + (jnl->jib_offset << HFSPLUS_SB(sb)->fs_shift));
		if (!jnl->jib_bh) {
			printk("HFS+-fs Line=%d: Error in buffer read\n", __LINE__);
			return;
		}
