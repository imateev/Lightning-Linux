...
reget:
	...
	if (error) {
        ...
        sdkp->usb_index = MAX_DISK_COUNT_PER_USB;
        if(2 == sdp->host->usb_id)
        {
        ...
        else if(3 == sdp->host->usb_id)
        {
            ...
            error = sd_format_disk_name("sdauralic", index, gd->disk_name, DISK_NAME_LEN);
 ...
 out_free_index:
        if(2 == sdp->host->usb_id)
                put_usb_disk_index(1, sdkp->usb_index);
        else if(3 == sdp->host->usb_id)
                put_usb_disk_index(0, sdkp->usb_index);
...
static void sd_info_shutdown(struct device *dev)
{
	struct scsi_disk *sdkp = scsi_disk_get_from_dev(dev);
...
static void scsi_disk_release(struct device *dev)
{
	...
	if(2 == sdkp->device->host->usb_id)
                put_usb_disk_index(1, sdkp->usb_index);
        else if(3 == sdkp->device->host->usb_id)
                put_usb_disk_index(0, sdkp->usb_index);
...
module_init(init_sd);
module_exit(exit_sd);
MODULE_AUTHOR("hu yongfa 2015-03-20");
